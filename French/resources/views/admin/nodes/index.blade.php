...
@section('content-header')
    <h1>Nodes<small>Tous les nodes disponibles sur le système.</small></h1>
    <ol class="breadcrumb">
    ...
    <div class="col-xs-12">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">Liste des nodes</h3>
                <div class="box-tools search01">
                    <form action="{{ route('admin.nodes') }}" method="GET">
                        <div class="input-group input-group-sm">
                            <input type="text" name="filter[name]" class="form-control pull-right" value="{{ request()->input('filter.name') }}" placeholder="Rechercher des nodes">
                            <div class="input-group-btn">
                                <button type="submit" class="btn btn-default"><i class="fa fa-search"></i></button>
                                <a href="{{ route('admin.nodes.new') }}"><button type="button" class="btn btn-sm btn-primary" style="border-radius: 0 3px 3px 0;margin-left:-1px;">Créer un nouveau</button></a>
                            </div>
                        </div>
                    </form>
                </div>
    ...
    (function pingNodes() {
        $('td[data-action="ping"]').each(function(i, element) {
            $.ajax({
                ...
                var errorText = 'Erreur de connexion au node! Vérifiez la console du navigateur pour plus de détails.';
                try {
