...
@section('content-header')
    <h1>{{ $server->name }}<small>Contrôler la commande de démarrage ainsi que les variables.</small></h1>
    <ol class="breadcrumb">
        ...
        <div class="col-xs-12">
            <div class="box box-primary">
                ...
                <div class="box-body">
                    ...
                    <p class="small text-muted">Modifiez la commande de démarrage de votre serveur ici. Les variables suivantes sont disponibles par défaut : <code>@{{SERVER_MEMORY}}</code>, <code>@{{SERVER_IP}}</code>, et <code>@{{SERVER_PORT}}</code>.</p>
                </div>
                <div class="box-body">
                    <label for="pDefaultStartupCommand" class="form-label">Commande de démarrage de service par défaut</label>
                ...
                <div class="box-body row">
                    <div class="col-xs-12">
                        <p class="small text-danger">
                            Le changement de l'une des valeurs ci-dessous entraînera l'exécution d'une commande de réinstallation du serveur. Le serveur sera arrêté puis procédera.
                            Si vous ne souhaitez pas que les scripts de service s'exécutent, assurez-vous que la case est cochée en bas.
                        </p>
                        <p class="small text-danger">
                            <strong>C'est une opération destructrice dans de nombreux cas. Ce serveur sera immédiatement arrêté pour que cette action puisse se poursuivre.</strong>
                        </p>
                    </div>
                    <div class="form-group col-xs-12">
                        ...
                        <p class="small text-muted no-margin">Sélectionnez le nest dans lequel ce serveur sera regroupé.</p>
                    </div>
                    <div class="form-group col-xs-12">
                        <label for="pEggId">Egg</label>
                        <select name="egg_id" id="pEggId" class="form-control"></select>
                        <p class="small text-muted no-margin">Sélectionnez l'Egg qui fournira les données de traitement pour ce serveur.</p>
                    </div>
                    <div class="form-group col-xs-12">
            ...
            <div class="box">
                ...
                <div class="box-body">
                    <div class="form-group">
                        ...
                        <input id="pDockerImageCustom" name="custom_docker_image" value="{{ old('custom_docker_image') }}" class="form-control" placeholder="Ou saisissez une image personnalisée..." style="margin-top:1rem"/>
                        <p class="small text-muted no-margin">C'est l'image Docker qui sera utilisée pour exécuter ce serveur. Sélectionnez une image dans la liste déroulante ou saisissez une image personnalisée dans le champ de texte ci-dessus.</p>
                    </div>
                </div>
            </div>
        </div>
...
@section('footer-scripts')
    ...
    $(document).ready(function () {
        $('#pEggId').select2({placeholder: 'Sélectionnez un Egg Nest'}).on('change', function () {
            ...
            if (!_.get(objectChain, 'startup', false)) {
                $('#pDefaultStartupCommand').val(_.get(parentChain, 'startup', 'ERREUR: Démarrage non défini !'));
            } else {
                ...
                var dataAppend = ' \
                    <div class="col-xs-12"> \
                        <div class="box"> \
                            ...
                            <div class="box-footer"> \
                                <p class="no-margin text-muted small"><strong>Variable de commande de démarrage :</strong> <code>' + item.env_variable + '</code></p> \
                                <p class="no-margin text-muted small"><strong>Règles d'entrée :</strong> <code>' + item.rules + '</code></p> \
                            </div> \
                        </div> \
                    </div>';
        ...
        $('#pNestId').select2({placeholder: 'Sélectionnez un Nest'}).on('change', function () {
            $('#pEggId').html('').select2({
